...
pub fn safe_divide_err(numerator: f64, denominator: f64) -> Result<f64, String> {
    if denominator == 0.0 {
...
pub fn safe_divide_ok(numerator: f64, denominator:f64) -> Result<f64,String> {
    if denominator == 0.0 {
       Err(String::from("Can not devide by zero"))
    } else {
        Ok(numerator/denominator)
    }
